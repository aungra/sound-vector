...
- `pcm_reversible_data`  
  復元用PCMを保持する見える保護図案。外周のmicro-weave / seal bandとして表示し、削除・変形しない。Illustratorでは表示したままロックする。
...
3. `01-print_area` と `10-pcm_reversible_data` のIllustratorレイヤーをロックする。`10-pcm_reversible_data` は表示したまま保護し、`02-axis_structure` は非表示にする。
4. `03-pcm_carriers`、`04-waveform_guides`、`05-spectral_guides`、`06-chroma_guides`、`07-onset_tempo_guides` を下絵にして描き直す。
5. 音の復元性を残す場合は `10-pcm_reversible_data` を表示したままロックし、削除・拡大縮小・回転・パス単純化をしない。
