...
6. `10-pcm_reversible_data` は表示したままロックする。
...
- `10-pcm_reversible_data.svg`  
  復元用PCMデータの見える保護レイヤー。実際の曲から書き出した可逆SVGでは、このレイヤーが主図案内の locked protected texture になり、音の復元情報を保持する。表示したままロックし、編集、削除、拡大縮小、回転、パス単純化、前後移動による分離をしない。
