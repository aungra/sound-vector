...
function loadPatternApi() {
  ...
  const appScript = scripts.at(-1).replace(
    ...
    "globalThis.__patternApi={genrePatternProfiles,musicGenreProfiles,resolveGenrePattern};"
  );
  const context = {
    console,
    Date,
...
test("genre pattern profiles cover all calibrated genre names", () => {
  const { genrePatternProfiles, musicGenreProfiles, resolveGenrePattern } = loadPatternApi();
